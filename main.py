"""Aggregates Ohio-located listings from every taildraggers.com
...
ones whose location ends in ", OH".
...
Jersey/New Mexico/New York/North Carolina/North Dakota repos' approach,
with the state filter swapped to "OH".
"""
...
PAGE_TITLE = "Taildragger Ads in Ohio"
STATE_ABBR = "OH"
...
_OHIO_LOCATION_RE = re.compile(r",\s*OH\s*$", re.IGNORECASE)


def _is_ohio(location: str) -> bool:
    return bool(_OHIO_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        ohio_rows = [row for row in rows if _is_ohio(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(ohio_rows)} in Ohio")
        all_listings.extend(ohio_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Ohio listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Ohio listings: {len(listings)}")
