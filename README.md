...
Daily aggregator of Ohio-located taildragger classified listings,
...
[North Dakota](https://github.com/taildraggers/north-dakota) repos'
approach, with the state filter swapped to "OH".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, OH` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Ohio"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/ohio/`
...
```html
<iframe
  src="https://taildraggers.github.io/ohio/"
  title="Taildragger Ads in Ohio"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_OHIO_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
